...
class LeituraArquivo:
    ...
    def BuildCSV(self, path_Base, arquivoSaida, listaCaracteristica, MATRIZ_FLORES):
        features = (listaCaracteristica.__len__() * self.tamanhoTransformada).__str__()
        arquivoSaida.write(
            self.numeroAmostras.__str__() + "," + features + "Iris-virginica, Iris-setosa, Iris-versicolor\n");
        self.CorpoArquivoCSV(path_Base, arquivoSaida, listaCaracteristica, MATRIZ_FLORES)
    ...
    def CorpoArquivoCSV(self, path_Base, arquivoSaida, listaCaracteristica, MATRIZ_FLORES):

        for contador, list in enumerate(MATRIZ_FLORES):
            matriz = matriz = numpy.array(list)
    ...
    def BuildCSV_WEKA(self, path_Base, arquivoSaida, listaCaracteristica, MATRIZ_PROTEINAS):
        features = (
        listaCaracteristica.__len__() * self.tamanhoTransformada)

        contador = 0
        for x in ERANGE(features + 1):
            if(x == 0):
                contador = contador + 1
        ...
        self.CorpoArquivoCSV_WEKA(path_Base, arquivoSaida, listaCaracteristica, MATRIZ_PROTEINAS)
